...
export default function WalletBar({contractBalance}) {
  const { requireInstall, balance } = useWeb3()
    ...
    <section className="text-white bg-indigo-600 rounded-lg">
      <div className="p-8">
        <div className="sm:flex justify-between items-top">
          <div className="text-base xs:text-xl mr-4 truncate">Hello, {account.data}</div>
          { network.hasInitialResponse && network.isSupported && account.data &&
            <div className="text-base xs:text-xl break-words">Smart contract balance = {` `}
            <strong>{contractBalance} ETH</strong></div>
          }
        </div>
